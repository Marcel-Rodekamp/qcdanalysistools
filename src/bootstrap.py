...
def bootstrap(t_data,t_blocked = False, t_num_blocks = None):
    """
    ...
    if t_blocked:
        ...
        for block_id in range(0,t_num_blocks):
            l_est[block_id] = bootstrap_est(blocking.get_block(t_data,block_id,block_size))
            l_var[block_id] = bootstrap_var(blocking.get_block(t_data,block_id,block_size))

        l_est[t_num_blocks-1] = bootstrap_est(blocking.get_block(t_data,t_num_blocks-1,block_size))
        l_var[t_num_blocks-1] = bootstrap_var(blocking.get_block(t_data,t_num_blocks-1,block_size))
