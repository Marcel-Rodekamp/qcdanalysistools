...
r"""
    ...
    correlation functions C(t). Thus it is assumed that for $t\to\infty$
        $$
            C(t) \sim A_0 e^{-t E_0} + A_0 e^{-(N_t - t) E_0}
                  = 2 A_0 e^{- \frac{N_t E_0}{2}} \cosh\left(\left( \frac{N_t}{2} - t \right) E_0\right)
        $$
    A periodic temporal direction is assumed
"""

def energy_firstorder(t_correlator, t_A0 = 1, t_E0=1,t_analysis_type="Plain", t_plot_path=None, **analysis_kwargs):
    r"""
    ...
    correlator_fct = lambda t,A_0,E_0 : \
        2*A_0*np.exp(-Nt*E_0/2)*np.cosh((Nt/2-t)*E_0)
    ...
    corr_fit = correlator_fct(t_axis, popt[0], popt[1])
    ...
    if t_plot_path is not None:
        ...
        plt.plot(t_axis,
                 corr_fit,
                 label = r"$ C(t) = 2 A_0 e^{{- \frac{{N_t E_0 }}{{2}} }} \cosh\left(\left( \frac{{N_t}}{{2}} - t \right) E_0\right)$, $_{{A_0 = {0:.2g},\, E_0 = {1:.2g} }}$".format(*popt))
