...
class AnalysisParam(dict):
    ...
    def _jak_init(self,**kwargs):
        ...
        if self.get('use_blocking'):
            ...
            if self.get('N_jak') != 1:
                raise AttributeError(f"Decide between use_blocking=True and N_jak > 1. The effect is the same.")
    ...
    def num_samples(self):
        if checkAnalysisType(self.AnalysisType,Bootstrap):
            ...
            return self.get('data_size')#-self.get('N_jak')
        elif checkAnalysisType(self.AnalysisType,Blocking):
    ...
    def get_remainder_dim(self,shape):
        remainder = list(shape)
        remainder.pop(self.get('axis',default=0))
        return *remainder,


def get_sample(t_param,t_data,t_k):
    def _get_bst_sample(t_data):
        return t_data.take(t_param.bst_table[t_k],axis=t_param['axis'])

    def _get_jak_sample(t_data):
        return np.delete( t_data,
    ...
    if checkAnalysisType(t_param.AnalysisType,Bootstrap):
        if t_param['use_blocking']:
            blk_sample = np.zeros(shape=(t_param['N_blk'], *t_param.get_remainder_dim(t_data.shape)) ,dtype=t_data.dtype)
            for k_blk in range(t_param['N_blk']):
                blk_sample[k_blk,...] = np.mean(_get_blk_sample(t_data,k_blk),axis=t_param['axis'])

            return _get_bst_sample(t_data=blk_sample)
        else:
    ...
    if checkAnalysisType(t_param.AnalysisType,Jackknife):
        if t_param['use_blocking']:
            ...
            return _get_jak_sample(t_data=blk_sample)
        else:
...
#         return np.array(
#             [
#                 np.mean(get_sample(t_param,t_data,k), axis=t_param['axis']) for k in range(t_param['N_bst'])
#             ]
#         )
#
#     def _jkn_resample():
...
def _jackknife_est(t_param,t_data,t_observable=None,**obs_kwargs):
    ...
    jkn_data = np.zeros(shape=(t_param.num_samples(), *t_param.get_remainder_dim(t_data.shape)), dtype=t_data.dtype)
    for k in t_param.iterate_samples():
        jkn_data[k,...] = np.mean(get_sample(t_param,obs,k),axis=t_param['axis'])
...
def _blocking_est(t_param,t_data,t_observable=None,**obs_kwargs):
    ...
    blk_data = np.zeros(shape=(t_param.num_samples(), *t_param.get_remainder_dim(t_data.shape)), dtype=t_data.dtype)
    for k in t_param.iterate_samples():
        blk_data[k,...] = np.mean(get_sample(t_param,obs,k), axis = t_param['axis'])

    return np.mean(blk_data,axis=0)
...
def _blocking(t_param,t_data,t_observable=None,**obs_kwargs):
    ...
    return np.mean(blk_data,axis=0), np.var(blk_data,axis=0)
